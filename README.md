...
The **Booking** tab (its own tab, separate from everything else) lets each rep
connect their own Google account and get a public booking link — a contact
picks a free slot, books it, and it lands straight on that rep's Google
Calendar with a confirmation email sent from their own connected Gmail
address.

### 1. Set up the Google OAuth client

1. **console.cloud.google.com** → APIs & Services → **Library** → enable both
   the **Google Calendar API** and the **Gmail API**.
2. APIs & Services → **Credentials** → Create Credentials → **OAuth client
   ID** → Application type **Web application**.
3. Add an **Authorized redirect URI**: `{your domain}/api/google-callback` —
   e.g. `https://your-app.vercel.app/api/google-callback` in production, or
   your ngrok URL + `/api/google-callback` in local dev (see `PUBLIC_URL` in
   `.env.example`; without a tunnel it falls back to
   `http://localhost:3001/api/google-callback`, which also works if that's
   what you register).
...
Same variables in both your local `.env` and Vercel's Project Settings →
Environment Variables (redeploy after adding them there).

### 3. Connect your Google account

Open the **Booking** tab → **Connect Google Calendar** → approve access.
Each rep does this for their own account — there's no shared calendar.

### 4. Choose which calendars count as "busy"

Once connected, the tab lists every calendar in that Google account with a
checkbox — only checked calendars are read for conflicts when working out
what's free. Untick any calendar you don't want blocking the booking page,
**including your main/primary calendar** if you'd rather it not be checked.
New bookings are added to whichever calendar is picked under "Add bookings
to" (defaults to your primary calendar) regardless of which ones are ticked
here — the two are independent.

### 5. Set the meeting length, hours, and share the link

- **Meeting length** — 15, 30 or 60 minutes, or hourly up to 8 hours.
- **Working hours** — per weekday, in your own timezone; only slots inside
  these hours (and outside any calendar conflict) are ever offered.
- **Minimum notice** / **how far ahead** bookings can be made.
- Your **booking link** (`/book/your-slug`) is right there to copy and send
  to a contact — share it however you'd share a Calendly link.

### How a booking works

1. The contact opens the link, picks **their own timezone** (auto-detected,
   editable) so times are never crossed, then a date and a free slot.
2. On submit, the backend re-checks the slot is still free (live
   `freeBusy` call against the calendars ticked above — nobody double-books
   a slot two people had open at once), creates the event on the host's
   Google Calendar, and emails the contact a confirmation **sent from the
   host's own connected Gmail address** via the Gmail API.
3. The booking shows up in the Booking tab's list, cancellable from there
   (which also removes the calendar event).

Availability is always read live from Google on every page load — any
change made directly in Google Calendar (a new meeting, a cancelled one) is
reflected immediately, and every booking made here is written straight back
to Google Calendar. There's no separate polling/webhook layer keeping a
copy in sync — it reads live and writes live, which is what keeps the two
sides from ever drifting apart.

### How it fits together

- `server/googleCalendar.js` — OAuth token exchange/refresh and the Calendar
  + Gmail REST calls (plain `fetch`, no Google SDK dependency).
- `server/bookingSlots.js` — pure date/timezone math: converts working hours
  in the host's timezone into UTC, and turns Google's busy intervals into
  offerable slots.
- `server/bookingApi.js` — the actual request handling, shared by both
  `server/index.js` (local dev) and the `/api/google-*.js`,
  `/api/booking-settings.js`, `/api/bookings.js`, `/api/public-*.js`
  functions (deployed site) — same split as everything else in this app.
- `src/BookingPage.jsx` — the public, unauthenticated booking page at
  `/book/:slug` (see `src/main.jsx` for how that's routed and
  `vercel.json` for the rewrite that makes it load on a fresh visit).
