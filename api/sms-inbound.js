...
export default async function handler(req, res) {
  ...
  // immediately — it needs no database access.
  res.setHeader("Content-Type", "text/xml");
  res.status(200).send("<Response></Response>");

  // Vercel can freeze this function the instant the response above is
  // sent — code after res.send() is NOT guaranteed to keep running on
  // its own. waitUntil() explicitly tells the platform to keep this
  // invocation alive until the promise settles, so the contact match
  // + conversation logging below reliably completes even though
  // Twilio has already moved on.
  waitUntil(
    (async () => {
      try {
        await ensureSchema();
