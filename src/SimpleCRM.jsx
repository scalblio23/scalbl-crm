...
import { getTimezoneList, timezoneOffsetLabel } from "./lib/timezones";
...
export default function SimpleCRM() {
  // Defaults to the "page" query param if one's there — that's how
  // /api/google-callback lands back on the Booking tab after a Google
  // Calendar connect/reconnect (see the googleConnectNotice effect below).
  const [page, setPage] = useState(
    () => new URLSearchParams(window.location.search).get("page") || "conversation"
  );
  ...
  // Each rep connects their own Google account and gets their own
  // booking page — see server/bookingApi.js for the read (live
  // freeBusy pull) and write (creates the calendar event + sends the
  // confirmation email from that rep's own Gmail) sides of this.
  const [bookingLoaded, setBookingLoaded] = useState(false);
  const [bookingSettings, setBookingSettings] = useState(null);
  const [bookingForm, setBookingForm] = useState(null); // editable draft, saved on demand
  const [bookingConnected, setBookingConnected] = useState(false);
  const [bookingGoogleEmail, setBookingGoogleEmail] = useState(null);
  const [bookingCalendars, setBookingCalendars] = useState([]);
  const [bookingCalendarsLoading, setBookingCalendarsLoading] = useState(false);
  const [bookingConnecting, setBookingConnecting] = useState(false);
  const [bookingSaving, setBookingSaving] = useState(false);
  const [bookingNotice, setBookingNotice] = useState(null); // { type: "success"|"error", text }
  ...
  const settingsToForm = (settings) => ({
    ...
    destinationCalendarId: settings.destination_calendar_id,
    workingHours: settings.working_hours,
  });

  const loadBookingSettings = async () => {
    try {
      const data = await api.get("/api/booking-settings");
      setBookingSettings(data.settings);
      setBookingForm(settingsToForm(data.settings));
      setBookingConnected(data.connected);
      setBookingGoogleEmail(data.googleEmail);
      setBookingCalendars(data.calendars || []);
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not load booking settings." });
    } finally {
  ...
  const loadBookings = async () => {
    ...
    try {
      setBookings(await api.get("/api/bookings"));
    } catch {
      // non-fatal — the settings above are the important part of this tab
    } finally {
  ...
  useEffect(() => {
    if (page === "booking" && authUser && authUser.role !== "client" && !bookingLoaded) {
      loadBookingSettings();
      loadBookings();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, authUser, bookingLoaded]);

  // One-time: pick up ?page=booking&google=connected|error&message=...
  // left by /api/google-callback after a Google Calendar connect
  // attempt, show a toast, and clean the URL so a refresh doesn't
  // re-trigger it.
  useEffect(() => {
    ...
    if (!google) return;
    if (google === "connected") {
      setBookingNotice({ type: "success", text: "Google Calendar connected." });
      setBookingLoaded(false); // reload settings/calendars now that a connection exists
    } else if (google === "error") {
      setBookingNotice({ type: "error", text: params.get("message") || "Could not connect Google Calendar." });
    }
    window.history.replaceState({}, "", window.location.pathname);
  }, []);

  const connectGoogleCalendar = async () => {
    setBookingConnecting(true);
    setBookingNotice(null);
    try {
      const { url } = await api.get("/api/google-connect");
      window.location.href = url; // full-page redirect into Google's consent screen
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not start the Google connection." });
      setBookingConnecting(false);
    }
  };
  ...
  const disconnectGoogleCalendar = async () => {
    if (
      !window.confirm(
        "Disconnect Google Calendar? Your booking page will stop accepting new bookings until you reconnect."
      )
    )
      return;
    try {
      await api.post("/api/google-disconnect", {});
      setBookingConnected(false);
      setBookingGoogleEmail(null);
      setBookingCalendars([]);
      setBookingNotice({ type: "success", text: "Google Calendar disconnected." });
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not disconnect Google Calendar." });
    }
  };

  const refreshBookingCalendars = async () => {
    setBookingCalendarsLoading(true);
    try {
      setBookingCalendars(await api.get("/api/google-calendars"));
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not load calendars." });
    } finally {
      setBookingCalendarsLoading(false);
    }
  };

  const toggleBookingCalendarIncluded = async (calendarId, included) => {
    setBookingCalendars((cals) => cals.map((c) => (c.calendar_id === calendarId ? { ...c, included } : c)));
    try {
      await api.patch("/api/google-calendars", { calendarId, included });
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not update that calendar." });
      refreshBookingCalendars(); // roll the checkbox back to whatever's actually saved
    }
  };
  ...
  const saveBookingSettings = async () => {
    ...
    setBookingNotice(null);
    try {
      const { settings } = await api.patch("/api/booking-settings", bookingForm);
      setBookingSettings(settings);
      setBookingForm(settingsToForm(settings));
      setBookingNotice({ type: "success", text: "Booking page updated." });
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not save booking settings." });
    } finally {
      setBookingSaving(false);
    }
  };

  const updateBookingWorkingDay = (dayKey, patch) => {
    setBookingForm((f) => ({
      ...f,
      workingHours: { ...f.workingHours, [dayKey]: { ...f.workingHours[dayKey], ...patch } },
    }));
  };

  const cancelBookingRow = async (id) => {
    if (!window.confirm("Cancel this booking? This also removes the event from the calendar.")) return;
    try {
      const updated = await api.patch("/api/bookings", { id, status: "cancelled" });
      setBookings((bs) => bs.map((b) => (b.id === id ? updated : b)));
    } catch (err) {
      setBookingNotice({ type: "error", text: err.message || "Could not cancel that booking." });
    }
  };

  const bookingLink = bookingSettings ? `${window.location.origin}/book/${bookingSettings.slug}` : "";
  const copyBookingLink = async () => {
    try {
      ...
      // clipboard access denied — the link is still shown/selectable in the input
    }
  };
  ...
  return (
    <div className="flex h-screen bg-white text-gray-900" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        ...
        {/* Booking — its own tab: Google Calendar connection, which
            calendars feed availability, the public booking page's
            settings, and the resulting bookings. Kept separate from
            Reports/Settings on purpose, per the ask. */}
        {page === "booking" && (
          <div className="flex-1 overflow-y-auto">
            <div className="px-8 py-6 border-b border-gray-100">
              <h1 className="text-xl font-bold">Booking</h1>
              <div className="text-sm text-gray-400 mt-0.5">
                Let contacts book time straight onto your Google Calendar — availability is
                read live from Google, and every booking is written straight back to it.
              </div>
            </div>
            ...
            <div className="p-8 max-w-3xl space-y-6">
              {bookingNotice && (
                ...
                >
                  ...
                  <div className="flex-1">{bookingNotice.text}</div>
                  <button onClick={() => setBookingNotice(null)} className="text-gray-400 hover:text-gray-600 shrink-0">
                    <X size={14} />
                  </button>
                </div>
              )}
              ...
              {!bookingLoaded ? (
                ...
                <>
                      ...
                      <div>
                        ...
                        {bookingConnected ? (
                          <div className="text-xs text-gray-500 mt-1 flex items-center gap-1.5">
                            <CheckCircle2 size={13} className="text-green-600" /> Connected as {bookingGoogleEmail}
                          </div>
                        ) : (
                          <div className="text-xs text-gray-400 mt-1">Not connected — bookings can't be taken yet.</div>
                        )}
                      </div>
                      {bookingConnected ? (
                        <button
                  ...
                  {bookingConnected && bookingSettings && bookingForm && (
                    <>
                      {/* Booking link */}
                      <div className="border border-gray-200 rounded-xl p-5">
                        <div className="text-sm font-semibold flex items-center gap-2 mb-2">
                          <Link2 size={16} /> Your booking link
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <input
                            readOnly
                            ...
                            className="flex-1 min-w-[16rem] border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-600"
                          />
                          <button
                            onClick={copyBookingLink}
                            className="flex items-center gap-1.5 border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium hover:bg-gray-50"
                          >
                            <Copy size={14} /> {bookingLinkCopied ? "Copied" : "Copy"}
                          </button>
                          <a
                            href={bookingLink}
                            target="_blank"
                      ...
                      {/* Calendars to pull availability from */}
                      <div className="border border-gray-200 rounded-xl p-5">
                        <div className="flex items-center justify-between mb-1">
                          <div className="text-sm font-semibold">Calendars to pull availability from</div>
                          <button
                            onClick={refreshBookingCalendars}
                            disabled={bookingCalendarsLoading}
                            className="flex items-center gap-1 text-xs font-medium text-gray-500 hover:text-gray-800"
                          >
                            <RefreshCw size={12} className={bookingCalendarsLoading ? "animate-spin" : ""} /> Refresh
                          </button>
                        </div>
                        <p className="text-xs text-gray-400 mb-3">
                          Only checked calendars are used to work out when you're busy — untick any you don't want
                          blocking your booking page, including your main calendar.
                        </p>
                        <div className="space-y-1">
                          {bookingCalendars.map((cal) => (
                            <label
                              key={cal.calendar_id}
                              className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-gray-50 text-sm cursor-pointer"
                            >
                              <input
                                type="checkbox"
                                checked={cal.included}
                                onChange={(e) => toggleBookingCalendarIncluded(cal.calendar_id, e.target.checked)}
                                className="rounded border-gray-300"
                              />
                              <span className="flex-1">{cal.calendar_summary}</span>
                              {cal.is_primary && (
                          ...
                          {bookingCalendars.length === 0 && (
                            <div className="text-xs text-gray-400 px-3 py-2">No calendars loaded yet — click Refresh.</div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="border border-gray-200 rounded-xl p-5 space-y-5">
                        <div className="text-sm font-semibold">Booking page</div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="col-span-2">
                            <label className="text-xs font-medium block mb-1.5 text-gray-500">Title</label>
                            <input
                          ...
                          <div className="col-span-2">
                            <label className="text-xs font-medium block mb-1.5 text-gray-500">Description</label>
                            <textarea
                          ...
                          <div>
                            <label className="text-xs font-medium block mb-1.5 text-gray-500">Location (optional)</label>
                            <input
                          ...
                          <div>
                            <label className="text-xs font-medium mb-1.5 text-gray-500 flex items-center gap-1">
                              <Clock size={12} /> Meeting length
                            </label>
                            <select
                              value={bookingForm.slotMinutes}
                              onChange={(e) => setBookingForm((f) => ({ ...f, slotMinutes: Number(e.target.value) }))}
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white outline-none focus:border-gray-400"
                            >
                              {BOOKING_SLOT_OPTIONS.map((m) => (
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setBookingForm((f) => ({ ...f, timezone: e.target.value }))}
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white outline-none focus:border-gray-400"
                            >
                          ...
                          <div>
                            <label className="text-xs font-medium block mb-1.5 text-gray-500">Add bookings to</label>
                            <select
                              value={bookingForm.destinationCalendarId}
                              onChange={(e) => setBookingForm((f) => ({ ...f, destinationCalendarId: e.target.value }))}
                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white outline-none focus:border-gray-400"
                            >
                              {bookingCalendars.map((cal) => (
                        ...
                        <div>
                          <div className="text-xs font-medium text-gray-500 mb-2">Working hours (your timezone)</div>
                          <div className="space-y-1.5">
                            {BOOKING_WEEKDAYS.map(([key, label]) => {
                              const day = bookingForm.workingHours?.[key] || { enabled: false, start: "09:00", end: "17:00" };
                              return (
                                <div key={key} className="flex items-center gap-3 text-sm">
                                  <label className="flex items-center gap-2 w-32 shrink-0">
                                    <input
                                      type="checkbox"
                                      checked={day.enabled}
                                      onChange={(e) => updateBookingWorkingDay(key, { enabled: e.target.checked })}
                                      className="rounded border-gray-300"
                                    />
                                    {label}
                                  </label>
                                  <input
                                    ...
                                    onChange={(e) => updateBookingWorkingDay(key, { start: e.target.value })}
                                    className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm disabled:opacity-40"
                                  />
                                  ...
                                  <input
                                    ...
                                    value={day.end}
                                    disabled={!day.enabled}
                                    onChange={(e) => updateBookingWorkingDay(key, { end: e.target.value })}
                                    className="border border-gray-200 rounded-lg px-2 py-1.5 text-sm disabled:opacity-40"
                                  />
                                </div>
                              );
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 bg-gray-900 text-white text-sm px-5 py-2.5 rounded-lg font-medium disabled:opacity-40"
                        >
                          {bookingSaving && <Loader2 size={14} className="animate-spin" />}
                          Save changes
                        </button>
                      </div>
                      ...
                      {/* Bookings */}
                      <div className="border border-gray-200 rounded-xl p-5">
                        <div className="text-sm font-semibold mb-3">Bookings</div>
                        {bookingsLoading ? (
                          <div className="flex items-center gap-2 text-sm text-gray-400">
                            <Loader2 size={14} className="animate-spin" /> Loading…
                          </div>
                        ) : bookings.length === 0 ? (
                          <div className="text-sm text-gray-400">No bookings yet — share your booking link above.</div>
                        ) : (
                          <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                              <thead>
                                    ...
                                    <td className="py-2.5 pr-4">
                                      <span
                                        className={`text-xs px-2 py-0.5 rounded-full ${
                                          b.status === "cancelled" ? "bg-gray-100 text-gray-400" : "bg-green-100 text-green-700"
                                        }`}
                                      >
