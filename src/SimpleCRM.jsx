...
export default function SimpleCRM() {
  ...
  // Leads for the powerdialler, newest first
  const dialQueue = [...contacts].sort(
    (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
  );
  const activeLead = dialQueue[dialIndex];
    ...
    <div className="flex h-screen bg-white text-gray-900" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        ...
        {page === "powerdialler" && (
          <div className="flex flex-1 overflow-hidden">
            <div className="w-80 border-r border-gray-200 flex flex-col">
              <div className="px-4 py-4 border-b border-gray-100 font-semibold flex items-center justify-between">
                <span>Leads</span>
              ...
              <div className="flex-1 overflow-y-auto">
                {dialQueue.map((lead, i) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">{lead.client}</div>
                    <span
                      className={`inline-block mt-1.5 text-xs px-2 py-0.5 rounded-full border ${statusColors[lead.status]}`}
                    >
                      {lead.status}
                    </span>
                  </button>
              ...
              <div className="w-full max-w-md">
                <div className="text-center mb-6">
                  ...
                  <div className="text-sm text-gray-500">
                    {dialIndex + 1} of {dialQueue.length} in queue
                  </div>
                </div>
                <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
                  <div className="text-2xl font-bold">{activeLead.name}</div>
                  <div className="text-gray-500 mt-1">{activeLead.phone}</div>
                  <div className="text-sm text-gray-500">{activeLead.email}</div>
                  <div className="text-sm text-gray-400 mt-0.5">{activeLead.client}</div>
                  <span className={`inline-block mt-3 text-xs px-2.5 py-1 rounded-full border ${statusColors[activeLead.status]}`}>
                    {activeLead.status}
                  </span>
                  {activeLead.notes && (
                    <div className="mt-4 text-left bg-gray-50 border border-gray-100 rounded-lg px-4 py-3 text-sm text-gray-600">
                  ...
                  <div className="mt-8 flex justify-center gap-4">
                    {!calling ? (
                      ...
                      <button
                        onClick={() => {
                          setCalling(false);
                          setDialIndex((i) => (i + 1) % dialQueue.length);
                        }}
                ...
                <button
                  onClick={() => setDialIndex((i) => (i + 1) % dialQueue.length)}
